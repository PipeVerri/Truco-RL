    ...
    def match(self, debug=False, seed=None):
        ...
        def process_action(i, action):
            ...
            if action < 3: # Tira alguna de las primeras 3 cartas
                ...
                assert card["id"] != -1
                agents[i].played_cards[mano] = card
                agents[i].played_cards[action]["id"] = -1
                ...
                if opponent_card["id"] != -1: # Si el otro ya jugo, actualizar el hand_state y fijarme si se definio la partida
                    ...
                    if mano_result == 3:
                        hand_state[mano] = 3
                    elif (mano_result == 1 and i == 0) or (mano_result == 2 and i == 1):
                        hand_state[mano] = 1
                    else:
                        hand_state[mano] = 2
                    ...
                    tie = mano_result == 3
                    my_critical_hand_state = build_hand_critical_input(action, hand_state, i, agents[i].is_mano)
                    if (not my_critical_hand_state[mano][0] and loose) or (not my_critical_hand_state[mano][1] and tie):
                        # Sumarle al oponente el truco + 1(por ganar)
                        agents[not i].points += truco_state + 1
                        return False
                    # Fijarme si la partida la gane
                    opp_critical_hand_state = build_hand_critical_input(action, hand_state, not i, agents[not i].is_mano)
                    if (not opp_critical_hand_state[mano][0] and win) or (not opp_critical_hand_state[mano][1] and tie):
                ...
                assert agents[i].can_cantar_truco and desired_truco_state == truco_state + 1 # Fijarme que lo tenga y que ese cantando algo correcto
                ...
                agents[winner].points += envido_points
                if falta_envido:
                ...
                if falta_envido:
                    agents[not i].points += calculate_falta_envido_points(agents[i].points)
